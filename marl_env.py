...
OBS_DIM = 45  # see _build_observation for the schema
# SIMPLE-MARKET branch: 21 base features (current-hour state + services) + 24
# perfect-foresight PUN prices for the whole day. The agent sees the TRUE energy
# price profile of the day (exactly what the MILP sees on the energy axis), but
# services remain current-hour only. No commitments, no day-ahead lead: bids are
# settled in the same hour (single-hour auction). This is the simplified market
# used to give the PPO a fair shot: full energy foresight, immediate settlement.
_PUN_FORESIGHT_HOURS = 24
...
class MultiBESSEnv(ParallelEnv):
    ...
    def _build_observation(self, agent: str) -> np.ndarray:
        ...
        # ---- 21-44: 24h perfect-foresight PUN prices ----
        # The agent sees the TRUE energy price for every hour of the day (same
        # information the MILP has on the energy axis), normalised like the
        # current-hour price. This is the key difference of the simple-market
        # branch: full energy foresight, no commitments, immediate settlement.
        H = self.episode_hours
        for h in range(_PUN_FORESIGHT_HOURS):
            hh = min(h, H - 1)
            obs[21 + h] = self._prices_episode[hh] / 200.0
