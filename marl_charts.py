# -*- coding: utf-8 -*-
"""
marl_charts.py — figure aggiuntive per il paper, generate dopo il production run.
...
UTILIZZO
--------
Le trajectories sono salvate da marl_pipeline quando si usa
...
Per generare queste figure SENZA rilanciare il pipeline, due opzioni:
 (a) modificare marl_run_experiment.py per aggiungere un torch.save delle
...
Per (a):
  python marl_charts.py results/<run_id>

Per (b): chiamato come callback alla fine di marl_run_experiment.py.
"""
...
def make_extra_charts(
    ...
    marl_run_experiment.py after run_full_pipeline() returns."""
    out_dir = Path(out_dir)
    print(f"\n[marl_charts] generating into {out_dir}/png + /pdf")
...
def main_cli():
    ...
    marl_run_experiment.py — see the README block at the top of this file).
    """
    if len(sys.argv) < 2:
        print("usage: python marl_charts.py results/<run_id>")
        print("       (requires trajectories.pkl in that folder)")
        sys.exit(1)
    run_dir = Path(sys.argv[1])
    pkl = run_dir / "trajectories.pkl"
    if not pkl.exists():
        print(f"ERROR: {pkl} not found. Add this snippet to marl_run_experiment.py")
