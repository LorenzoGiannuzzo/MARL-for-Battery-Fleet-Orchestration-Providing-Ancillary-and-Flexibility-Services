...
def make_paper_charts(
    ...
    # ---- 19. Overcommit / non-delivery penalties per policy ----
    # Only drawn if the trajectories carry the penalty series (runs produced
    # after penalty tracking was added). Shows total euros of penalty each
    # policy incurred over the test window: a direct view of overcommitment.
    if all("penalty_per_hour" in trajectories[p] for p in policies):
        fig, ax = plt.subplots(figsize=(8, 5))
        labels = [_POLICY_LABELS[p] for p in policies]
        totals = [float(np.sum(trajectories[p]["penalty_per_hour"])) for p in policies]
        colors = [_POLICY_COLORS[p] for p in policies]
        bars = ax.bar(labels, totals, color=colors)
        ax.set_ylabel("Total penalties over test window (EUR)")
        ax.set_title("Overcommit / non-delivery penalties by policy")
        ax.grid(axis="y", alpha=0.3)
        for b, v in zip(bars, totals):
            ax.text(b.get_x() + b.get_width() / 2, b.get_height(),
                    f"{v:,.0f}", ha="center", va="bottom", fontsize=9)
        fig.tight_layout()
        _save(fig, "overcommit_penalties", dpi=120)
