...
class MILPOptimizer:
    ...
    def _extract_results(self, variables: Dict, time_horizon: int, 
        ...
        # Calculate flexibility service revenues
        flexibility_revenue = 0.0
        for t in range(time_horizon):
            services = flexibility_services[t]
            ...
            for service in services:
                if service.service_type == ServiceType.FCR:
                    capacity_rev = variables['R_fcr'][t].varValue * service.capacity_price
                    energy_rev = (variables['A_fcr'][t].varValue * service.energy_price * 
                                service.activation_probability)
                    flexibility_revenue += capacity_rev + energy_rev
                    
                elif service.service_type == ServiceType.AFRR:
                    capacity_rev = variables['R_afrr'][t].varValue * service.capacity_price
                    energy_rev = (variables['A_afrr'][t].varValue * service.energy_price * 
                                service.activation_probability)
                    flexibility_revenue += capacity_rev + energy_rev
                    
                elif service.service_type == ServiceType.MFRR:
                    capacity_rev = variables['R_mfrr'][t].varValue * service.capacity_price
                    energy_rev = (variables['A_mfrr'][t].varValue * service.energy_price * 
                                service.activation_probability)
                    flexibility_revenue += capacity_rev + energy_rev
